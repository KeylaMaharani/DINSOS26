...
<head>
  ...
  <title>Riwayat Pengajuan DTSEN</title>
  ...
  <main class="max-w-4xl mx-auto px-4 py-8">
    <div class="bg-white rounded-xl shadow-sm p-6 md:p-8">
      ...
      <div class="flex items-center justify-between mb-5">
        <h1 class="text-lg font-bold text-[#121d26]">Riwayat Pengajuan DTSEN</h1>
        <a href="{{ route('layanan.dtsen.ajukan') }}" class="bg-primary text-white text-sm font-bold rounded-full px-5 py-2 hover:bg-secondary transition">
          + Ajukan Baru
        </a>
      </div>
      ...
      @if ($data->isEmpty())
        <p class="text-sm text-gray-500 text-center py-10">Belum ada pengajuan DTSEN.</p>
      @else
          ...
          <table class="w-full text-sm text-left">
            <thead class="text-xs text-gray-500 uppercase border-b">
              <tr>
                ...
                <th class="py-2 pr-4">Alasan Cetak</th>
            ...
            <tbody class="divide-y">
              @foreach ($data as $item)
                <tr>
                  ...
                  <td class="py-3 pr-4">{{ $item->alasan_cetak }}</td>
                  <td class="py-3 pr-4">
                    @php $labels = \App\Models\DtsenPermohonan::statusLabels(); @endphp
