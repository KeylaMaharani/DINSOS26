...
<div class="space-y-5">
    ...
    <form method="GET" class="bg-surface-container-lowest border border-outline-variant/40 rounded-xl p-4 flex flex-wrap items-end gap-3">
        ...
        @if (request()->hasAny(['search', 'status']))
            <a href="{{ route('dtsen.ajuan') }}" class="px-4 py-2 rounded-lg text-sm text-on-surface-variant hover:text-primary">
                Reset
            </a>
        @endif
    </form>
    ...
    <div class="bg-surface-container-lowest border border-outline-variant/40 rounded-xl overflow-hidden">
        ...
        <table class="w-full text-sm">
            <thead class="bg-surface-container text-on-surface-variant text-[11px] uppercase">
                <tr>
                    <th class="text-center px-4 py-3 whitespace-nowrap">NIK</th>
                    <th class="text-center px-4 py-3 whitespace-nowrap">Nama</th>
            ...
            <tbody class="divide-y divide-outline-variant/30">
                @forelse ($ajuanList as $item)
                    <tr class="hover:bg-surface-container-low transition-colors">
                        <td class="px-4 py-3 font-medium text-primary whitespace-nowrap text-center">{{ $item->nik }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-center">{{ $item->nama_pemohon }}</td>
                        <td class="px-4 py-3 text-on-surface-variant max-w-[220px] truncate text-center" title="{{ $item->alamat }}">{{ $item->alamat }}</td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <a href="{{ route('dtsen.show', $item) }}"
                                class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-primary/10 text-primary hover:bg-primary hover:text-on-primary text-xs font-medium transition-colors whitespace-nowrap">
