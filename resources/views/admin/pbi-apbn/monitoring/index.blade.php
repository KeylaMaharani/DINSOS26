        ...
        @forelse ($data as $item)
            ...
            <div class="bg-surface-container-lowest border border-outline-variant/40 rounded-xl overflow-hidden">
                <div class="flex flex-wrap gap-x-8 gap-y-1 px-4 py-2.5 border-b border-outline-variant/40 text-xs">
                    <span><span class="text-on-surface-variant">No. Registrasi:</span> <span class="font-medium text-primary">{{ $item->no_registrasi }}</span></span>
                    <span><span class="text-on-surface-variant">Nama Pemohon:</span> <span class="font-medium">{{ $item->nama_kepala_keluarga }}</span></span>
                    <span><span class="text-on-surface-variant">Desil:</span> <span class="font-medium">{{ $item->desil_nasional ?: '-' }}</span></span>
                    <a href="{{ route('pbi-apbn.ajuan.show', $item) }}" class="ml-auto inline-flex items-center gap-1 text-primary hover:underline font-medium">
                    ...
                    <tbody>
                        ...
                        @forelse ($item->logs as $i => $log)
                            <tr class="{{ $isRejected ? 'bg-error-container text-on-error-container' : 'bg-success-container text-on-success-container' }}">
                                <td class="px-3 py-2 whitespace-nowrap">{{ \Carbon\Carbon::parse($log->created_at)->format('d-m-Y H:i') }}</td>
                                <td class="px-3 py-2 truncate" title="{{ $log->task_name }} ({{ $log->role_name }})">
                                    {{ $log->task_name }} ({{ strtolower($log->role_name) }})
                                </td>
